...
const openSpotifyHomeButton = document.getElementById("openSpotifyHome");
...
function bindEvents() {
  ...
  spotifyForm.addEventListener("submit", handleSpotifySearch);
  openSpotifyHomeButton.addEventListener("click", () => loadBrowserUrl("https://open.spotify.com"));
...
function handleSpotifySearch(event) {
  event.preventDefault();
  const query = toText(document.getElementById("spotifyQuery").value);
  const url = query
